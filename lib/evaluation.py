...
def encode_data(model, data_loader, log_step=10, logging=logger.info):
    ...
    img_embs = None
    cap_embs = None
    img_cap_entities_embs = None
    text_entities_embs = None



    for i, data_i in enumerate(data_loader):
        # make sure val logger is used
        images, img_lengths, captions, image_cap_embeddings, bge_cap_embs, padded_text_entities, text_entity_lengths,padded_text_relations, lengths, ids, img_ids, repeat = data_i
        model.logger = val_logger

        # compute the embeddings
        img_emb, cap_emb,  text_entities_emb = model.forward_emb(
            images, captions, image_cap_embeddings, bge_cap_embs,  padded_text_entities,
             text_entity_lengths,  padded_text_relations, lengths, image_lengths=img_lengths)

        if img_embs is None:
            dataset_size = len(data_loader.dataset)
            img_embs = np.zeros((dataset_size, img_emb.size(1)))
            cap_embs = np.zeros((dataset_size, cap_emb.size(1)))
            # img_cap_entities_embs = np.zeros((dataset_size, img_emb.size(1)))
            text_entities_embs = np.zeros((dataset_size, cap_emb.size(1)))
        # cache embeddings
        img_embs[ids] = img_emb.data.cpu().numpy().copy()
        cap_embs[ids, :] = cap_emb.data.cpu().numpy().copy()
        # img_cap_entities_embs[ids] = img_cap_entities_emb.data.cpu().numpy().copy()
        text_entities_embs[ids, :] = text_entities_emb.data.cpu().numpy().copy()
    ...
    return img_embs, cap_embs, text_entities_embs
...
def compute_sims(img_embs, cap_embs, img_lens, cap_lens, model, shard_size=128):
    """
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            il = torch.Tensor(il).cuda()
            sl = torch.Tensor(sl).cuda()
...
def evalrank(model_path, opt=None, tokenizer=None, model=None, split='dev', fold5=False, save_path=None, data_path=None):
    ...
    with torch.no_grad():
        img_embs, cap_embs, text_entities_embs = encode_data(model, data_loader)
    ...
    # sims = g_sims_softmax #533.3
    # sims= e_sims_softmax #424.3
    #532.0，test_5 538.4
